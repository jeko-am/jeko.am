...
export async function createOrder(orderData: OrderData): Promise<CreatedOrder> {
  try {
    ...
    const { data: order, error: orderError } = await supabase
      .from('orders')
      .insert({
        customer_id: null, // Guest checkout - no customer_id
        ...
        payment_status: 'paid', // Directly mark as paid for guest checkout
        notes: `Payment method: ${orderData.payment_method}. Customer: ${orderData.customer_first_name} ${orderData.customer_last_name}, Email: ${orderData.customer_email}, Phone: ${orderData.customer_phone}, Shipping Address: ${JSON.stringify(orderData.shipping_address)}, Billing Address: ${JSON.stringify(orderData.shipping_address)}`,
      })
      .select()
      .single();
    ...
    // Create customer record for guest checkout
    const { data: customer, error: customerError } = await supabase
      .from('customers')
      .insert({
        user_id: null, // Guest checkout - no user_id
